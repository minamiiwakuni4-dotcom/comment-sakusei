...
import {
  buildClamassAiUserPrompt,
  clamassMultimodalSystemPrompt,
} from "@/lib/clamass-ai-prompt";
...
export async function POST(request: Request) {
  ...
  const textPack = formData.get("textPack");
  if (typeof textPack !== "string" || !textPack.trim()) {
    return NextResponse.json(
      { error: "textPack（評価テキスト）が必要です。" },
      { status: 400 },
    );
  }
  ...
  if (imageParts.length === 0) {
    return NextResponse.json(
      { error: "ノートの写真を1枚以上アップロードしてください（最大3枚）。" },
      { status: 400 },
    );
  }

  const userText = buildClamassAiUserPrompt(textPack.trim(), imageParts.length);
  ...
  try {
    ...
    const model = genAI.getGenerativeModel({
      ...
      systemInstruction: {
        role: "system",
        parts: [{ text: clamassMultimodalSystemPrompt() }],
      },
    });
